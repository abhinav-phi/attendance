...
app.get('/api/init', async (req, res) => {
    try {
        ...
        try {
            ...
            console.log('student_login110.php length:', html.length);
        } catch(e) {
        ...
        if (!html || html.length < 100) {
            try {
                ...
                html = loginRes.data;
                console.log('student_login.php length:', html.length);
            } catch(e) {
        ...
        if (!captchaPath) {
            console.log('Getting captcha from refresh endpoint...');
            const refreshRes = await client.get(`${BASE_URL}/plum5_fw_utils.php?rty=captcha&typ=login`, {
                headers: getHeaders(`${BASE_URL}/student_login.php`)
            });
            const refreshMatch = refreshRes.data.match(/images\/captcha\/captcha_\d+\.jpg/i);
            if (refreshMatch) {
...
app.post('/api/login', async (req, res) => {
    ...
    try {
        ...
        const loginRes = await client.post(`${BASE_URL}/student_login.php`, formData, {
            headers: { 
                ...getHeaders(`${BASE_URL}/student_login.php`),
        ...
        if (lowerHtml.includes('invalid') || lowerHtml.includes('incorrect') || 
            lowerHtml.includes('wrong captcha')) {
            console.log('❌ Login failed - error detected in response');
            return res.status(401).json({ error: "Invalid credentials or captcha. Please try again." });
        }
        ...
        if (loginHtml.includes('frameset') || loginHtml.includes('plum_url') || 
            loginHtml.includes('Welcome') || loginHtml.includes('Logout') || 
        ...
        return res.status(401).json({ error: "Login failed. Please check credentials and captcha." });
...
async function fetchAttendance(client, username, loginResponseHtml) {
    ...
    while (allFrames.length > 0 && !attendanceUrl) {
        ...
        try {
            ...
            if (frameHtml.includes('Session expired') || frameHtml.includes('No Rights')) {
            ...
            $frame('a').each((i, el) => {
                ...
                if (href.includes('plum_url')) {
                    ...
                    if (textLower.includes('attendance') || 
                        ...
                        attendanceUrl = href.startsWith('http') ? href : `${BASE_URL}/${href}`;
    ...
    if (formRes.data.includes('Session expired') || formRes.data.includes('No Rights')) {
    ...
    $form('select').each((i, el) => {
        ...
        if (name && name !== 'year' && name !== 'sem') {
            ...
            formData[name] = selected.length ? selected.val() : $form(el).find('option').first().val();
    ...
    if (attRes.data.includes('Session expired') || attRes.data.includes('No Rights')) {
...
app.post('/api/attendance', async (req, res) => {
    ...
    try {
        const attendance = await fetchAttendance(client, session.username, session.loginHtml);
